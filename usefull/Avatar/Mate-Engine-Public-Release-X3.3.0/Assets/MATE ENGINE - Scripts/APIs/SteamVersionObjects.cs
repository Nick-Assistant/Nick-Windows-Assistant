...
public class SteamVersionObjects : MonoBehaviour
{
    ...
    {
        for (int i = 0; i < steamOnlyObjects.Count; i++) if (steamOnlyObjects[i]) notSteamObjects[i].SetActive(isSteam);
        for (int i = 0; i < notSteamObjects.Count; i++) if (notSteamObjects[i]) steamOnlyObjects[i].SetActive(!isSteam);
    }
}
